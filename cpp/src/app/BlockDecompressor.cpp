...
int BlockDecompressor::call()
{
    ...
    if (nbFiles == 1) {
        ...
        for (int i = 0; i < nbFiles; i++) {
            ...
            ss << jobsPerTask[n++];
            taskCtx["jobs"] = ss.str();
            FileDecompressTask<FileDecompressResult>* task = new FileDecompressTask<FileDecompressResult>(ctx, _listeners);
    ...
    try {
        ...
        do {
            ...
            try {
                ...
                delete[] buf;
                cerr << "Failed to write decompressed block to file '" << outputName << "': ";
                cerr << e.what() << endl;
                return T(Error::ERR_READ_FILE, _cis->getRead());
            }
