...
func BenchmarkMurMurHash3(b *testing.B) {
	...
	hash, err := hash.NewMurMurHash3(0)

	if err != nil {
		msg := fmt.Sprintf("Failed to create MurMurHash3: %v\n", err)
		b.Errorf(msg)
	}
	...
	if res != 3774107959 {
		b.Errorf("Incorrect result for MurMurHash3")
	}
}

func BenchmarkSipHash_2_4(b *testing.B) {
	buffer := make([]byte, 1024*1024)
	...
	if err != nil {
		msg := fmt.Sprintf("Failed to create SipHash_2_4: %v\n", err)
		b.Errorf(msg)
	}
	...
	for i := 0; i < iter; i++ {
		hash.SetSeedFromLongs(uint64(i), uint64(0))
		res += hash.Hash(buffer)
	}

	if res != 15907504797896139582 {
		b.Errorf("Incorrect result for SipHash_2_4")
	}
}
