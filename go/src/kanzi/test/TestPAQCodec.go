...
func TestCorrectness() {
	...
	for ii := 1; ii < 20; ii++ {
		...
		pc, _ := entropy.NewBinaryEntropyEncoder(dbgbs, predictor1)

		if _, err := pc.Encode(values); err != nil {
		...
		pd, _ := entropy.NewBinaryEntropyDecoder(dbgbs2, predictor2)

		ok := true
		values2 := make([]byte, len(values))
		if _, err := pd.Decode(values2); err != nil {
...
func TestSpeed() {
	...
	for jj := 0; jj < 3; jj++ {
		...
		for ii := 0; ii < iter; ii++ {
			...
			pc, _ := entropy.NewBinaryEntropyEncoder(obs, predictor1)

			// Encode
			before := time.Now()

			if _, err := pc.Encode(values1); err != nil {
		...
		for ii := 0; ii < iter; ii++ {
			...
			pd, _ := entropy.NewBinaryEntropyDecoder(ibs, predictor2)
