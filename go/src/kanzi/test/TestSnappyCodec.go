...
func TestCorrectness() {
	...
	for ii := 0; ii < 20; ii++ {
		...
		_, _, err = snappy.Inverse(output[0:dstIdx], reverse)
...
func TestSpeed() {
	...
	for jj := 0; jj < 3; jj++ {
		...
		for ii := 0; ii < iter; ii++ {
			...
			if _, _, err = snappy.Inverse(output[0:dstIdx], reverse); err != nil {
		...
		fmt.Printf("\nSnappy encoding [ms]: %v", delta1/1000000)
		fmt.Printf("\nThroughput [MB/s]   : %d", prod*1000000/delta1*1000/(1024*1024))
		fmt.Printf("\nSnappy decoding [ms]: %v", delta2/1000000)
